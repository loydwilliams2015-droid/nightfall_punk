...
Date: 2026-08-14
Source baseline: `build/v1.2b-camera-rebuild`
Status: **DESIGN RESOLUTION / FUTURE v1.4 PLANNING**
Compile status: **no compile implied**.

## Governing thesis

v1.4 should generalize relations that v1.2/v1.2b already prove rather than add parallel authorities or feature-count complexity.
...
- 6 former `ASK` decisions, now resolved below by user acceptance of recommendations.
...
## Six formerly open questions — now resolved
...
The new fixed spatial statement is the **8 m x 8 m base Southern-California grid element**. Larger partitions are integer/structured aggregations chosen from evidence.
...
1. Preserve v1.2b camera, movement, server authority, teammate causality and negative-control proofs.
...
- **chronotope:** place is topography + time/history + recurring activity, while topology captures the exchange/dependency structures that may become part of the place's organization;
...
No decision in this ledger claims v1.4 code already exists.

## Closed design status

The six former questions are now dispositioned according to the accepted recommendations. The v1.4 **design-resolution layer is therefore closed at 150/150 questions**, while implementation constants, profiling thresholds and human tuning remain open and evidence-driven.
