...
int main(int argc,char **argv) {
    ...
    const int width=1280,height=720; InitWindow(width,height,"nightfall!punk v0.4 combat lab"); SetTargetFPS(144);
    ...
    while(!WindowShouldClose()) {
        ...
        DrawText("nightfall!punk v0.4 - COMBAT LAB",24,18,23,RAYWHITE);DrawText("WASD move | Shift sprint | Ctrl crouch | Space jump | E ladder | LMB fire | R reload | 1/2 weapons | F10 mouse",24,47,15,GRAY);const char *status=welcomed?"AUTHORITATIVE + PREDICTED":(connected?"HANDSHAKE":"RECONNECTING");DrawText(TextFormat("NET %s | ENet RTT %u ms | effective ping %u ms | snapshot age %u ms",status,peer?nf_net_peer_rtt_ms(peer):0,last_pong_ms,last_snapshot_ms?now-last_snapshot_ms:0),24,79,16,welcomed?GREEN:YELLOW);DrawText(TextFormat("server tick %llu | client tick %llu | cmd %u ack %u pending %u",(unsigned long long)last_server_tick,(unsigned long long)world.tick,prediction.next_sequence?prediction.next_sequence-1:0,prediction.last_acknowledged,(prediction.next_sequence-1)-prediction.last_acknowledged),24,101,16,RAYWHITE);DrawText(TextFormat("prediction %.3fm | max %.3fm | corrections %u | traversal disagreements %u",prediction.last_error,prediction.max_error,prediction.corrections,candidate_disagreements),24,123,16,RAYWHITE);
