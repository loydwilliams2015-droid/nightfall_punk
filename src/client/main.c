...
static Color actor_color(NfFaction local,NfFaction remote,bool alive) { if(!alive)return(Color){70,72,78,255}; if(remote==NF_FACTION_RANCHER)return(Color){238,139,38,255}; NfRelationship relation=nf_relation_between(local,remote,NF_RELATION_HOSTILE); if(relation==NF_RELATION_COOPERATIVE)return(Color){65,145,235,255}; if(relation==NF_RELATION_TRUCE||relation==NF_RELATION_CONTESTED)return(Color){224,188,70,255}; return(Color){220,72,72,255}; }
...
static void draw_region_graph(const NfRegionGraph *graph) { if(graph==NULL)return; for(size_t i=0;i<graph->count;++i){const NfRegion *r=&graph->regions[i];Vector3 a={r->center.x,0.18f,r->center.z};DrawSphere(a,1.25f,(Color){70,205,210,210});for(uint8_t n=0;n<r->neighbor_count;++n){uint8_t j=r->neighbors[n];if(j<=i||j>=graph->count)continue;const NfRegion *b=&graph->regions[j];DrawLine3D(a,(Vector3){b->center.x,0.18f,b->center.z},(Color){70,205,210,135});}} }
static void draw_remote_actor(Vector3 p,const RemoteActor *remote,NfFaction local_faction) { if(remote==NULL)return; const bool cattler=remote->faction==NF_FACTION_RANCHER; float body_h=remote->alive?(cattler?NF_CATTLER_BODY_HEIGHT:1.80f):0.25f; float body_w=cattler?NF_CATTLER_BODY_WIDTH:0.72f; Color color=actor_color(local_faction,remote->faction,remote->alive); DrawCube((Vector3){p.x,p.y+body_h*0.5f,p.z},body_w,body_h,body_w,color); DrawCubeWires((Vector3){p.x,p.y+body_h*0.5f,p.z},body_w,body_h,body_w,RAYWHITE); if(cattler&&remote->alive){Color weak=(Color){255,222,86,235};DrawCube((Vector3){p.x,p.y+NF_CATTLER_KNEE_BAND_CENTER_Y,p.z},NF_CATTLER_WEAK_WIDTH,NF_CATTLER_KNEE_BAND_HEIGHT,NF_CATTLER_WEAK_WIDTH,weak);DrawCube((Vector3){p.x,p.y+NF_CATTLER_FOOT_BAND_CENTER_Y,p.z},NF_CATTLER_WEAK_WIDTH,NF_CATTLER_FOOT_BAND_HEIGHT,NF_CATTLER_WEAK_WIDTH,weak);} }
...
int main(int argc,char **argv) {
    ...
    const int width=1280,height=720; InitWindow(width,height,"nightfall!punk v0.8 dream cattler habitat ecology lab"); SetTargetFPS(144);
    ...
    while(!WindowShouldClose()) {
        ...
        now=nf_net_now_ms();
        if(IsKeyPressed(KEY_F4))spatial_debug=!spatial_debug;
