...
static void draw_remote_actor(Vector3 p,const RemoteActor *remote,NfFaction local_faction) { if(remote==NULL)return; const bool cattler=remote->faction==NF_FACTION_RANCHER; float body_h=remote->alive?(cattler?2.80f:1.80f):0.25f; float body_w=cattler?0.84f:0.72f; Color color=actor_color(local_faction,remote->faction,remote->alive); DrawCube((Vector3){p.x,p.y+body_h*0.5f,p.z},body_w,body_h,body_w,color); DrawCubeWires((Vector3){p.x,p.y+body_h*0.5f,p.z},body_w,body_h,body_w,RAYWHITE); if(cattler&&remote->alive){Color weak=(Color){255,222,86,235};DrawCube((Vector3){p.x,p.y+0.56f,p.z},body_w+0.08f,0.09f,body_w+0.08f,weak);DrawCube((Vector3){p.x,p.y+0.15f,p.z},body_w+0.08f,0.08f,body_w+0.08f,weak);} }
