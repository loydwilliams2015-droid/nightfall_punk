...
static NfVec3 dummy_spawn(size_t index) {
    static const NfVec3 p[NF_DUMMY_COUNT]={{0,0.05f,-6},{0,0.05f,6}};
    return p[index%NF_DUMMY_COUNT];
}
static NfFaction dummy_faction(size_t index) { return index==0u?NF_FACTION_RIVAL:NF_FACTION_PLAYER; }
...
static bool respawn_position(const NfServerClient clients[],NfEntityId id,NfVec3 *out) {
    for(size_t i=0;i<NF_SERVER_CLIENTS;++i) {
        if(clients[i].occupied&&clients[i].entity_id==id) { *out=spawn_for_slot(i); return true; }
    }
    for(size_t i=0;i<NF_DUMMY_COUNT;++i) {
        if(g_dummy_ids[i]==id) { *out=dummy_spawn(i); return true; }
    }
    return false;
}
...
static void broadcast_event(NfNetHost *net,NfServerClient clients[],NfCombatEvent event) {
    ...
    for(size_t i=0;i<NF_SERVER_CLIENTS;++i) {
        if(clients[i].occupied&&clients[i].connected) nf_net_send(net,clients[i].peer,reliable?NF_NET_CHANNEL_RELIABLE:NF_NET_CHANNEL_STATE,buf,n,reliable);
    }
}
...
static bool hitscan_target(const NfWorld *world,const NfHistoryFrame *frame,NfEntityId shooter,NfFaction shooter_faction,NfVec3 origin,NfVec3 dir,float range,bool friendly_fire,NfEntityId *target_out,NfHitZone *zone_out,float *distance_out) {
    ...
    for(uint8_t i=0;i<frame->count;++i) {
        ...
        const NfActor *current=nf_world_find_actor_const(world,h->id); if(current==NULL||!nf_combat_can_damage(shooter_faction,current->faction,friendly_fire))continue;
...
static void process_combat_for_client(NfNetHost *net,NfWorld *world,NfServerClient clients[],NfServerClient *c,NfHistoryFrame history[],bool friendly_fire) {
    ...
    if(c->combat_input.weapon_slot>0u&&c->combat_input.weapon_slot<NF_WEAPON_COUNT&&nf_combat_select_weapon(actor,(NfWeaponId)c->combat_input.weapon_slot)) {
        NfCombatEvent e={.server_tick=world->tick,.type=NF_COMBAT_EVENT_WEAPON_SWITCH,.source=actor->id,.weapon=(NfWeaponId)c->combat_input.weapon_slot,.position=actor->transform.position}; broadcast_event(net,clients,e);
    }
    ...
    if(nf_combat_try_fire(actor,&c->combat_input,c->last_input_sequence,world->tick,&fire_event)) {
        broadcast_event(net,clients,fire_event); const NfWeaponSpec *spec=nf_weapon_spec(actor->combat.weapon); float yaw=c->current_input.yaw_radians,pitch=c->combat_input.aim_pitch_radians,cp=cosf(pitch);
        ...
        const NfHistoryFrame *frame=history_find(history,c->current_client_tick,world->tick); NfEntityId target_id=0; NfHitZone zone=NF_HIT_NONE; float distance=0.0f;
        if(hitscan_target(world,frame,actor->id,actor->faction,origin,dir,spec->range,friendly_fire,&target_id,&zone,&distance)) {
            ...
            if(nf_combat_apply_damage(target,actor->id,actor->combat.weapon,zone,damage,world->tick,&hit_event)) {
                uint64_t rewind=frame&&world->tick>frame->tick?world->tick-frame->tick:0; uint64_t rewind_ms=(rewind*1000u)/NF_TICK_RATE; if(rewind_ms>65535u)rewind_ms=65535u; hit_event.rewind_ms=(uint16_t)rewind_ms; broadcast_event(net,clients,hit_event);
            }
        }
    }
...
static void process_respawns(NfNetHost *net,NfWorld *world,NfServerClient clients[]) {
    for(size_t i=0;i<NF_MAX_ENTITIES;++i) {
        NfActor *a=&world->actors[i]; if(!a->active||a->combat.alive||a->combat.respawn_timer>0.0f)continue; NfVec3 spawn; if(!respawn_position(clients,a->id,&spawn))continue;
        NfCombatEvent e={0}; nf_combat_respawn(a,spawn,world->tick,&e); reset_movement_after_respawn(world,a); broadcast_event(net,clients,e);
    }
}
...
int main(int argc,char **argv) {
    ...
    for(size_t i=0;i<NF_DUMMY_COUNT;++i) g_dummy_ids[i]=nf_world_spawn_actor(&world,dummy_faction(i),dummy_spawn(i));
