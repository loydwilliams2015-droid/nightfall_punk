...
void nf_claim_expire(NfClaimTable *table, uint64_t now_tick) {
    ...
    for (size_t i = 0u; i < NF_CLAIM_CAPACITY; ++i) {
        ...
        if (claim->active && now_tick >= claim->expires_tick) {
...
static NfClaim *free_slot(NfClaimTable *table) {
    for (size_t i = 0u; i < NF_CLAIM_CAPACITY; ++i) {
        if (!table->claims[i].active) return &table->claims[i];
    }
    return NULL;
}
...
NfClaimResult nf_claim_try_acquire(
    ...
    if (table == NULL || kind == NF_CLAIM_NONE || claimant == 0u ||
        subject_key == 0u || duration_ticks == 0u) {
    ...
    for (size_t i = 0u; i < NF_CLAIM_CAPACITY; ++i) {
        ...
        if (claim->strength == NF_CLAIM_HARD || strength == NF_CLAIM_HARD) {
            return NF_CLAIM_RESULT_BLOCKED;
        }
        shared = true;
    }
    ...
    *slot = (NfClaim){
        ...
        .expires_tick = now_tick + duration_ticks
    };
...
bool nf_claim_release(
    ...
    if (table == NULL || kind == NF_CLAIM_NONE || claimant == 0u || subject_key == 0u) return false;
    bool released = false;
    for (size_t i = 0u; i < NF_CLAIM_CAPACITY; ++i) {
        NfClaim *claim = &table->claims[i];
        if (claim->active && claim->kind == kind && claim->claimant == claimant &&
...
bool nf_claim_is_blocked(
    ...
    if (table == NULL || kind == NF_CLAIM_NONE || subject_key == 0u) return false;
    for (size_t i = 0u; i < NF_CLAIM_CAPACITY; ++i) {
        const NfClaim *claim = &table->claims[i];
        if (!claim->active || claim->kind != kind || claim->subject_key != subject_key ||
            now_tick >= claim->expires_tick) continue;
        if (claim->claimant != claimant && claim->strength == NF_CLAIM_HARD) return true;
    }
    return false;
}
...
NfEntityId nf_claim_hard_owner(
    ...
    uint64_t now_tick) {
    if (table == NULL || kind == NF_CLAIM_NONE || subject_key == 0u) return 0u;
    for (size_t i = 0u; i < NF_CLAIM_CAPACITY; ++i) {
        const NfClaim *claim = &table->claims[i];
        if (claim->active && claim->kind == kind && claim->subject_key == subject_key &&
            claim->strength == NF_CLAIM_HARD && now_tick < claim->expires_tick) {
...
size_t nf_claim_live_count(const NfClaimTable *table, uint64_t now_tick) {
    ...
    for (size_t i = 0u; i < NF_CLAIM_CAPACITY; ++i) {
        const NfClaim *claim = &table->claims[i];
        if (claim->active && now_tick < claim->expires_tick) ++count;
    }
    return count;
}
...
const char *nf_claim_kind_name(NfClaimKind kind) {
    switch (kind) {
        case NF_CLAIM_COVER: return "COVER";
        case NF_CLAIM_TRAVERSAL: return "TRAVERSAL";
        case NF_CLAIM_INTERACTION: return "INTERACTION";
        case NF_CLAIM_REVIVE: return "REVIVE";
        default: return "NONE";
    }
}
...
const char *nf_claim_result_name(NfClaimResult result) {
    switch (result) {
        case NF_CLAIM_RESULT_GRANTED: return "GRANTED";
        case NF_CLAIM_RESULT_SHARED: return "SHARED";
        case NF_CLAIM_RESULT_BLOCKED: return "BLOCKED";
