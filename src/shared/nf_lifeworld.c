...
void nf_lifeworld_init(
    ...
    system->half_x=NF_LIFEWORLD_DEMO_HALF_X;
    system->half_z=NF_LIFEWORLD_DEMO_HALF_Z;
    system->seed=seed;
...
void nf_lifeworld_tick(
    ...
    system->cattlers_present=0u;
    system->humans_present=0u;
    for(size_t i=0u;i<NF_MAX_ENTITIES;++i){
        ...
        if(actor->faction==NF_FACTION_RANCHER){
            if(system->cattlers_present<255u)++system->cattlers_present;
            system->memory.last_cattler_tick=world->tick;
        }else if(actor->faction==NF_FACTION_RIVAL||actor->faction==NF_FACTION_PLAYER||actor->faction==NF_FACTION_TEAMMATE){
            if(system->humans_present<255u)++system->humans_present;
            system->memory.last_human_tick=world->tick;
        }
    }
    ...
    if(system->cattlers_present>0u){
        ...
        int loss=1;
        if(system->memory.maintenance_q>500u)loss=2;
    ...
    int throughput_change=(int)system->pump.throughput_q-(int)system->previous_pump_throughput_q;
    if(throughput_change<0)throughput_change=-throughput_change;
...
size_t nf_lifeworld_write_primary_json(
    ...
    if(n<0||(size_t)n>=cap){out[0]='\0';return 0u;}
    return(size_t)n;
}

const char *nf_life_activity_name(NfLifeActivity activity){
    switch(activity){
        case NF_LIFE_TRAVEL:return"TRAVEL";
        case NF_LIFE_WORK:return"WORK";
        case NF_LIFE_MAINTAIN:return"MAINTAIN";
        case NF_LIFE_OCCUPY:return"OCCUPY";
...
const char *nf_hotspot_name(NfEpistemicHotspotType hotspot){
    switch(hotspot){
        case NF_HOTSPOT_NONE:return"NONE";
        case NF_HOTSPOT_CONTESTED_OCCUPATION:return"CONTESTED_OCCUPATION";
        case NF_HOTSPOT_ACCESS_CHANGE:return"ACCESS_CHANGE";
        case NF_HOTSPOT_RECOVERY_EDGE:return"RECOVERY_EDGE";
        default:return"UNKNOWN";
    }
}
