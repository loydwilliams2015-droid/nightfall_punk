...
static float authority_error(
    ...
    float e=fabsf(a->focus_amount-b->focus_amount);
    for(int i=1;i<NF_WEAPON_COUNT;++i){
        const float d=fabsf(a->instability_deg[i]-b->instability_deg[i]);
        if(d>e)e=d;
    }
    const float rd=fabsf(a->redirect_stress_deg-b->redirect_stress_deg);
    const float sd=fabsf(a->support_stress_deg-b->support_stress_deg);
    if(rd>e)e=rd;if(sd>e)e=sd;
    if(a->accepted_shot_sequence!=b->accepted_shot_sequence)e+=1.0f;
    if(a->weapon!=b->weapon || a->weapon_state!=b->weapon_state)e+=1.0f;
    return e;
}
...
void nf_prediction_simulate_command(
    ...
    if(command->combat.weapon_slot>0u && command->combat.weapon_slot<NF_WEAPON_COUNT) {
        (void)nf_combat_select_weapon(actor,(NfWeaponId)command->combat.weapon_slot);
    }
    if(command->combat.reload_pressed) (void)nf_combat_start_reload(actor);

    nf_weapon_authority_step_actor(
        actor,command->combat.focus_held,1.0f/(float)NF_TICK_RATE);
    (void)nf_weapon_authority_try_fire(
        actor,&command->combat,command->sequence,world->tick,world->seed,
        command->move.yaw_radians,NULL,NULL);
}
...
bool nf_prediction_reconcile_v16b(
    ...
    if(auth_error>p->max_authority_error)p->max_authority_error=auth_error;
    ...
    const bool positional_correction = have_prediction && error > position_threshold;
    ...
    if(authority!=NULL) nf_weapon_authority_apply_net_state(actor,authority);
...
bool nf_prediction_reconcile(
    ...
    return nf_prediction_reconcile_v16b(
        p,world,id,authoritative,NULL,acknowledged,server_tick,threshold,0.08f);
}
