...
static void place_gate(NfRouteSystem *route, NfWorld *world, bool open) {
    if (route == NULL || world == NULL || route->gate_collider < 0 || (size_t)route->gate_collider >= world->collider_count) return;
    ...
    gate->previous_min=gate->min; gate->previous_max=gate->max;
    gate->base_min=gate->min; gate->base_max=gate->max;
}
...
void nf_route_init_v12_slice(NfRouteSystem *route, NfWorld *world) {
    ...
    route->closed_min=(NfVec3){-6.0f,0.0f,4.0f}; route->closed_max=(NfVec3){6.0f,2.2f,5.0f};
    route->open_min=(NfVec3){8.0f,0.0f,4.0f}; route->open_max=(NfVec3){20.0f,2.2f,5.0f};
    route->gate_center=(NfVec3){0.0f,1.1f,4.5f};
    route->route_a_entry=(NfVec3){-13.0f,0.05f,-2.0f}; route->route_a_reconnect=(NfVec3){-13.0f,0.05f,18.0f};
    route->route_b_entry=(NfVec3){0.0f,0.05f,1.2f}; route->route_b_reconnect=(NfVec3){0.0f,0.05f,18.0f};
    route->goal=(NfVec3){0.0f,0.05f,29.0f};
    route->gate_collider=nf_world_add_collider(world,NF_COLLIDER_SOLID,route->closed_min,route->closed_max);
...
bool nf_route_try_alter(NfRouteSystem *route, NfWorld *world, NfEntityId actor_id) {
    ...
    if (actor == NULL || !actor->active || !actor->combat.alive || actor->faction != NF_FACTION_PLAYER || !actor->input.interact_held) return false;
    ...
    route->open=true;
    ++route->revision; if (route->revision == 0u) route->revision=1u;
    route->changed_tick=world->tick; route->changed_by=actor_id;
...
void nf_route_apply_replica(NfRouteSystem *route, NfWorld *world, bool open, uint64_t changed_tick, NfEntityId changed_by) {
    ...
    if (route->open != open) { place_gate(route,world,open); ++route->revision; if(route->revision==0u)route->revision=1u; }
    route->open=open; route->changed_tick=changed_tick; route->changed_by=changed_by;
}
