...
static uint64_t get_u64(const uint8_t *p) { uint64_t v=0; for (unsigned i=0;i<8;++i) p[i]=p[i],v|=(uint64_t)p[i]<<(8u*i); return v; }
...
static size_t write_header(uint8_t *out, size_t cap, NfMessageType type) {
    if (out==NULL || cap<NF_HEADER_BYTES) return 0;
    put_u32(out,NF_PROTOCOL_MAGIC);
    put_u16(out+4,NF_PROTOCOL_VERSION);
    out[6]=(uint8_t)type;
    out[7]=0;
    return NF_HEADER_BYTES;
}
...
NfMessageType nf_protocol_peek_type(const uint8_t *data,size_t size) {
    if (!read_header(data,size,NF_MSG_NONE)) return NF_MSG_NONE;
    return (NfMessageType)data[6];
}

size_t nf_protocol_encode_hello(uint8_t *out,size_t cap,const NfHelloMessage *msg) {
    const size_t need=NF_HEADER_BYTES+NF_NET_NONCE_BYTES+NF_NET_TOKEN_BYTES;
    if (msg==NULL || cap<need) return 0;
    write_header(out,cap,NF_MSG_HELLO);
    memcpy(out+8,msg->client_nonce,NF_NET_NONCE_BYTES);
    memcpy(out+8+NF_NET_NONCE_BYTES,msg->resume_token,NF_NET_TOKEN_BYTES);
    return need;
}

bool nf_protocol_decode_hello(const uint8_t *data,size_t size,NfHelloMessage *out) {
    const size_t need=NF_HEADER_BYTES+NF_NET_NONCE_BYTES+NF_NET_TOKEN_BYTES;
    if (out==NULL || size!=need || !read_header(data,size,NF_MSG_HELLO)) return false;
    memcpy(out->client_nonce,data+8,NF_NET_NONCE_BYTES);
    memcpy(out->resume_token,data+8+NF_NET_NONCE_BYTES,NF_NET_TOKEN_BYTES);
    return true;
}

size_t nf_protocol_encode_welcome(uint8_t *out,size_t cap,const NfWelcomeMessage *msg) {
    const size_t need=NF_HEADER_BYTES+4+8+4+NF_NET_TOKEN_BYTES+1;
    if (msg==NULL || cap<need) return 0;
    write_header(out,cap,NF_MSG_WELCOME);
    size_t o=8;
    put_u32(out+o,msg->entity_id); o+=4;
    put_u64(out+o,msg->server_tick); o+=8;
    put_u32(out+o,msg->world_seed); o+=4;
    memcpy(out+o,msg->session_token,NF_NET_TOKEN_BYTES); o+=NF_NET_TOKEN_BYTES;
    out[o++]=msg->strong_crypto?1u:0u;
    return o;
}

bool nf_protocol_decode_welcome(const uint8_t *data,size_t size,NfWelcomeMessage *out) {
    const size_t need=NF_HEADER_BYTES+4+8+4+NF_NET_TOKEN_BYTES+1;
    if (out==NULL || size!=need || !read_header(data,size,NF_MSG_WELCOME)) return false;
    size_t o=8;
    out->entity_id=get_u32(data+o); o+=4;
    out->server_tick=get_u64(data+o); o+=8;
    out->world_seed=get_u32(data+o); o+=4;
    memcpy(out->session_token,data+o,NF_NET_TOKEN_BYTES); o+=NF_NET_TOKEN_BYTES;
    out->strong_crypto=data[o]!=0;
    return true;
}

size_t nf_protocol_encode_reject(uint8_t *out,size_t cap,uint8_t reason) {
    if (cap<NF_HEADER_BYTES+1) return 0;
    write_header(out,cap,NF_MSG_REJECT);
    out[8]=reason;
    return 9;
}

bool nf_protocol_decode_reject(const uint8_t *data,size_t size,uint8_t *reason) {
    if (reason==NULL || size!=9 || !read_header(data,size,NF_MSG_REJECT)) return false;
    *reason=data[8];
    return true;
}

static size_t encode_command(uint8_t *out,size_t cap,const NfInputCommand *c) {
    if (cap<NF_COMMAND_BYTES || c==NULL) return 0;
    size_t o=0;
    put_u32(out+o,c->sequence); o+=4;
    put_u64(out+o,c->client_tick); o+=8;
    put_f32(out+o,c->move.forward); o+=4;
    put_f32(out+o,c->move.strafe); o+=4;
    put_f32(out+o,c->move.yaw_radians); o+=4;
    uint8_t move_flags=0;
    if(c->move.jump_pressed)move_flags|=1u;
    if(c->move.crouch_held)move_flags|=2u;
    if(c->move.sprint_held)move_flags|=4u;
    if(c->move.interact_held)move_flags|=8u;
    out[o++]=move_flags;
    uint8_t combat_flags=0;
...
static bool decode_command(const uint8_t *data,size_t size,NfInputCommand *c) {
    if (size<NF_COMMAND_BYTES || c==NULL) return false;
    size_t o=0;
    memset(c,0,sizeof(*c));
    c->sequence=get_u32(data+o); o+=4;
    c->client_tick=get_u64(data+o); o+=8;
    c->move.forward=get_f32(data+o); o+=4;
    c->move.strafe=get_f32(data+o); o+=4;
    c->move.yaw_radians=get_f32(data+o); o+=4;
    uint8_t f=data[o++];
    c->move.jump_pressed=(f&1u)!=0;
    c->move.crouch_held=(f&2u)!=0;
    c->move.sprint_held=(f&4u)!=0;
    c->move.interact_held=(f&8u)!=0;
    f=data[o++];
...
size_t nf_protocol_encode_input(uint8_t *out,size_t cap,const NfInputBundle *msg) {
    if (msg==NULL || msg->count==0 || msg->count>NF_NET_INPUT_REDUNDANCY) return 0;
    const size_t need=NF_HEADER_BYTES+NF_NET_TOKEN_BYTES+1+(size_t)msg->count*NF_COMMAND_BYTES;
    if (cap<need) return 0;
    write_header(out,cap,NF_MSG_INPUT);
    size_t o=8;
    memcpy(out+o,msg->session_token,NF_NET_TOKEN_BYTES); o+=NF_NET_TOKEN_BYTES;
    out[o++]=msg->count;
    for (uint8_t i=0;i<msg->count;++i) o+=encode_command(out+o,cap-o,&msg->commands[i]);
    return o;
}

bool nf_protocol_decode_input(const uint8_t *data,size_t size,NfInputBundle *out) {
    if (out==NULL || size<NF_HEADER_BYTES+NF_NET_TOKEN_BYTES+1 || !read_header(data,size,NF_MSG_INPUT)) return false;
    size_t o=8;
    memset(out,0,sizeof(*out));
    memcpy(out->session_token,data+o,NF_NET_TOKEN_BYTES); o+=NF_NET_TOKEN_BYTES;
    out->count=data[o++];
    if (out->count==0 || out->count>NF_NET_INPUT_REDUNDANCY || size!=o+(size_t)out->count*NF_COMMAND_BYTES) return false;
    for (uint8_t i=0;i<out->count;++i) {
        if(!decode_command(data+o,size-o,&out->commands[i])) return false;
        o+=NF_COMMAND_BYTES;
    }
    return true;
}
...
static size_t encode_actor(uint8_t *out,size_t cap,const NfActorNetState *a) {
    if (cap<NF_ACTOR_BYTES || a==NULL) return 0;
    size_t o=0;
    put_u32(out+o,a->id); o+=4;
    out[o++]=(uint8_t)a->faction;
    put_f32(out+o,a->position.x); o+=4;
    put_f32(out+o,a->position.y); o+=4;
    put_f32(out+o,a->position.z); o+=4;
    put_f32(out+o,a->velocity.x); o+=4;
    put_f32(out+o,a->velocity.y); o+=4;
    put_f32(out+o,a->velocity.z); o+=4;
    out[o++]=(uint8_t)a->mode;
    uint8_t flags=(a->grounded?1u:0u)|(a->crouched?2u:0u);
    out[o++]=flags;
    out[o++]=a->jumps_used;
    put_u32(out+o,(uint32_t)a->attached_collider); o+=4;
    out[o++]=(uint8_t)a->candidate_type;
    ...
    put_u16(out+o,a->reserve_ammo); o+=2;
    return o;
}

static bool decode_actor(const uint8_t *data,size_t size,NfActorNetState *a) {
    if (size<NF_ACTOR_BYTES || a==NULL) return false;
    size_t o=0;
    memset(a,0,sizeof(*a));
    a->id=get_u32(data+o); o+=4;
    a->faction=(NfFaction)data[o++];
    a->position.x=get_f32(data+o); o+=4;
    a->position.y=get_f32(data+o); o+=4;
    a->position.z=get_f32(data+o); o+=4;
    a->velocity.x=get_f32(data+o); o+=4;
    a->velocity.y=get_f32(data+o); o+=4;
    a->velocity.z=get_f32(data+o); o+=4;
    a->mode=(NfMovementMode)data[o++];
    uint8_t f=data[o++];
    a->grounded=(f&1u)!=0;
    a->crouched=(f&2u)!=0;
    a->jumps_used=data[o++];
    a->attached_collider=(int32_t)get_u32(data+o); o+=4;
    a->candidate_type=(NfTraversalType)data[o++];
...
size_t nf_protocol_encode_ping(uint8_t *out,size_t cap,NfMessageType type,const NfPingMessage *msg) {
    if(msg==NULL || (type!=NF_MSG_PING&&type!=NF_MSG_PONG) || cap<NF_HEADER_BYTES+4) return 0;
    write_header(out,cap,type);
    put_u32(out+8,msg->stamp_ms);
    return 12;
}

bool nf_protocol_decode_ping(const uint8_t *data,size_t size,NfMessageType expected,NfPingMessage *out) {
    if(out==NULL || size!=12 || !read_header(data,size,expected)) return false;
    out->stamp_ms=get_u32(data+8);
    return true;
}

size_t nf_protocol_encode_combat_event(uint8_t *out,size_t cap,const NfCombatEventMessage *msg) {
    if(msg==NULL || cap<NF_COMBAT_EVENT_BYTES) return 0;
    write_header(out,cap,NF_MSG_COMBAT_EVENT);
    const NfCombatEvent *e=&msg->event;
    size_t o=8;
    put_u32(out+o,e->sequence); o+=4;
    put_u64(out+o,e->server_tick); o+=8;
    out[o++]=(uint8_t)e->type;
    put_u32(out+o,e->source); o+=4;
    put_u32(out+o,e->target); o+=4;
    out[o++]=(uint8_t)e->weapon;
    out[o++]=(uint8_t)e->hit_zone;
    put_f32(out+o,e->amount); o+=4;
    put_f32(out+o,e->position.x); o+=4;
...
bool nf_protocol_decode_combat_event(const uint8_t *data,size_t size,NfCombatEventMessage *out) {
    if(out==NULL || size!=NF_COMBAT_EVENT_BYTES || !read_header(data,size,NF_MSG_COMBAT_EVENT)) return false;
    memset(out,0,sizeof(*out));
    NfCombatEvent *e=&out->event;
    size_t o=8;
    e->sequence=get_u32(data+o); o+=4;
    e->server_tick=get_u64(data+o); o+=8;
    e->type=(NfCombatEventType)data[o++];
    e->source=get_u32(data+o); o+=4;
    e->target=get_u32(data+o); o+=4;
    e->weapon=(NfWeaponId)data[o++];
    e->hit_zone=(NfHitZone)data[o++];
    e->amount=get_f32(data+o); o+=4;
...
void nf_actor_to_net_state(const NfActor *actor,NfActorNetState *out) {
    ...
    memset(out,0,sizeof(*out));
    *out=(NfActorNetState){
        .id=actor->id,.faction=actor->faction,.position=actor->transform.position,.velocity=actor->transform.velocity,
...
void nf_actor_apply_combat_net_state(NfActor *actor,const NfActorNetState *s) {
    if(actor==NULL || s==NULL) return;
    ...
    if(s->weapon>NF_WEAPON_NONE && s->weapon<NF_WEAPON_COUNT) {
        actor->combat.ammo_mag[s->weapon]=s->ammo_mag;
        actor->combat.reserve_ammo[s->weapon]=s->reserve_ammo;
    }
}

void nf_actor_apply_net_state(NfActor *actor,const NfActorNetState *s,const NfMovementConfig *cfg) {
    if(actor==NULL || s==NULL || cfg==NULL) return;
    actor->transform.position=s->position;
    actor->transform.velocity=s->velocity;
    actor->movement.mode=s->mode;
    actor->movement.grounded=s->grounded;
    actor->movement.crouched=s->crouched;
    actor->movement.jumps_used=s->jumps_used;
    actor->movement.attached_collider=s->attached_collider;
    actor->movement.body_height=s->crouched?cfg->crouch_height:cfg->stand_height;
    actor->movement.eye_height=s->crouched?cfg->crouch_eye_height:cfg->stand_eye_height;
    actor->movement.candidate.active=s->candidate_type!=NF_TRAVERSAL_NONE;
