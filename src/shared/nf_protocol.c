...
static size_t write_header(uint8_t *out, size_t cap, NfMessageType type) {
    ...
    put_u32(out,NF_PROTOCOL_MAGIC); put_u16(out+4,NF_PROTOCOL_VERSION); out[6]=(uint8_t)type; out[7]=0;
...
size_t nf_protocol_encode_hello(uint8_t *out,size_t cap,const NfHelloMessage *msg) {
    ...
    write_header(out,cap,NF_MSG_HELLO); memcpy(out+8,msg->client_nonce,NF_NET_NONCE_BYTES); memcpy(out+8+NF_NET_NONCE_BYTES,msg->resume_token,NF_NET_TOKEN_BYTES); return need;
}
bool nf_protocol_decode_hello(const uint8_t *data,size_t size,NfHelloMessage *out) {
    ...
    memcpy(out->client_nonce,data+8,NF_NET_NONCE_BYTES); memcpy(out->resume_token,data+8+NF_NET_NONCE_BYTES,NF_NET_TOKEN_BYTES); return true;
}
...
size_t nf_protocol_encode_welcome(uint8_t *out,size_t cap,const NfWelcomeMessage *msg) {
    ...
    if (msg==NULL || cap<need) return 0;
    write_header(out,cap,NF_MSG_WELCOME); size_t o=8; put_u32(out+o,msg->entity_id); o+=4; put_u64(out+o,msg->server_tick); o+=8; put_u32(out+o,msg->world_seed); o+=4; memcpy(out+o,msg->session_token,NF_NET_TOKEN_BYTES); o+=NF_NET_TOKEN_BYTES; out[o++]=msg->strong_crypto?1u:0u; return o;
}
bool nf_protocol_decode_welcome(const uint8_t *data,size_t size,NfWelcomeMessage *out) {
    ...
    size_t o=8; out->entity_id=get_u32(data+o); o+=4; out->server_tick=get_u64(data+o); o+=8; out->world_seed=get_u32(data+o); o+=4; memcpy(out->session_token,data+o,NF_NET_TOKEN_BYTES); o+=NF_NET_TOKEN_BYTES; out->strong_crypto=data[o]!=0; return true;
}

size_t nf_protocol_encode_reject(uint8_t *out,size_t cap,uint8_t reason) {
    if (cap<NF_HEADER_BYTES+1) return 0;
    write_header(out,cap,NF_MSG_REJECT); out[8]=reason; return 9;
}
...
size_t nf_protocol_encode_input(uint8_t *out,size_t cap,const NfInputBundle *msg) {
    ...
    write_header(out,cap,NF_MSG_INPUT); size_t o=8; memcpy(out+o,msg->session_token,NF_NET_TOKEN_BYTES); o+=NF_NET_TOKEN_BYTES; out[o++]=msg->count;
...
bool nf_protocol_decode_input(const uint8_t *data,size_t size,NfInputBundle *out) {
    ...
    size_t o=8; memset(out,0,sizeof(*out)); memcpy(out->session_token,data+o,NF_NET_TOKEN_BYTES); o+=NF_NET_TOKEN_BYTES; out->count=data[o++];
    if (out->count==0 || out->count>NF_NET_INPUT_REDUNDANCY || size!=o+(size_t)out->count*NF_COMMAND_BYTES) return false;
    for (uint8_t i=0;i<out->count;++i) { if(!decode_command(data+o,size-o,&out->commands[i])) return false; o+=NF_COMMAND_BYTES; }
    return true;
}

static size_t encode_actor(uint8_t *out,size_t cap,const NfActorNetState *a) {
    if (cap<NF_ACTOR_BYTES || a==NULL) return 0;
    size_t o=0; put_u32(out+o,a->id); o+=4; out[o++]=(uint8_t)a->faction;
    ...
    put_f32(out+o,a->health); o+=4; out[o++]=a->alive?1u:0u; out[o++]=(uint8_t)a->weapon; out[o++]=(uint8_t)a->weapon_state; put_u16(out+o,a->ammo_mag); o+=2; put_u16(out+o,a->reserve_ammo); o+=2; return o;
}
static bool decode_actor(const uint8_t *data,size_t size,NfActorNetState *a) {
    if (size<NF_ACTOR_BYTES || a==NULL) return false;
    size_t o=0; memset(a,0,sizeof(*a)); a->id=get_u32(data+o); o+=4; a->faction=(NfFaction)data[o++];
...
size_t nf_protocol_encode_snapshot(uint8_t *out,size_t cap,const NfSnapshotMessage *msg) {
    ...
    const size_t need=NF_HEADER_BYTES+8+4+1+(size_t)msg->actor_count*NF_ACTOR_BYTES; if(cap<need)return 0;
    write_header(out,cap,NF_MSG_SNAPSHOT); size_t o=8; put_u64(out+o,msg->server_tick); o+=8; put_u32(out+o,msg->acknowledged_input); o+=4; out[o++]=msg->actor_count;
    for(uint8_t i=0;i<msg->actor_count;++i)o+=encode_actor(out+o,cap-o,&msg->actors[i]); return o;
}
bool nf_protocol_decode_snapshot(const uint8_t *data,size_t size,NfSnapshotMessage *out) {
    ...
    size_t o=8; memset(out,0,sizeof(*out)); out->server_tick=get_u64(data+o); o+=8; out->acknowledged_input=get_u32(data+o); o+=4; out->actor_count=data[o++];
    if(out->actor_count>NF_NET_MAX_SNAPSHOT_ACTORS || size!=o+(size_t)out->actor_count*NF_ACTOR_BYTES)return false;
    for(uint8_t i=0;i<out->actor_count;++i){if(!decode_actor(data+o,size-o,&out->actors[i]))return false;o+=NF_ACTOR_BYTES;} return true;
}

size_t nf_protocol_encode_ping(uint8_t *out,size_t cap,NfMessageType type,const NfPingMessage *msg) {
    if(msg==NULL || (type!=NF_MSG_PING&&type!=NF_MSG_PONG) || cap<NF_HEADER_BYTES+4)return 0; write_header(out,cap,type); put_u32(out+8,msg->stamp_ms); return 12;
}
bool nf_protocol_decode_ping(const uint8_t *data,size_t size,NfMessageType expected,NfPingMessage *out) {
    if(out==NULL || size!=12 || !read_header(data,size,expected))return false; out->stamp_ms=get_u32(data+8); return true;
}
...
size_t nf_protocol_encode_combat_event(uint8_t *out,size_t cap,const NfCombatEventMessage *msg) {
    ...
    write_header(out,cap,NF_MSG_COMBAT_EVENT); const NfCombatEvent *e=&msg->event; size_t o=8;
...
bool nf_protocol_decode_combat_event(const uint8_t *data,size_t size,NfCombatEventMessage *out) {
    ...
    memset(out,0,sizeof(*out)); NfCombatEvent *e=&out->event; size_t o=8;
