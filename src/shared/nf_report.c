...
static float sanitize_precision(float precision_m) {
    if (!isfinite(precision_m) || precision_m < NF_REPORT_MIN_PRECISION_M) {
        return NF_REPORT_MIN_PRECISION_M;
    }
    return precision_m;
}

static float quantize(float value, float precision_m) {
    return roundf(value / precision_m) * precision_m;
}

static NfVec3 quantize_position(NfVec3 position, float precision_m) {
    position.x = quantize(position.x, precision_m);
    position.y = quantize(position.y, precision_m);
    position.z = quantize(position.z, precision_m);
    return position;
}
...
bool nf_report_publish(NfReportBus *bus, NfReport report, uint32_t *id_out) {
    ...
    if (report.expiry_tick <= report.deliver_tick) {
        report.expiry_tick = report.deliver_tick + NF_REPORT_DEFAULT_LIFETIME_TICKS;
    }

    report.confidence = clamp01(report.confidence);
    report.origin_precision_m = sanitize_precision(report.origin_precision_m);
    report.precision_m = maxf(
        sanitize_precision(report.precision_m),
        report.origin_precision_m);
...
bool nf_report_relay(
    ...
    uint32_t *id_out) {
    if (bus == NULL || parent == NULL || reporter == 0u || parent->kind == NF_REPORT_NONE) {
        return false;
    }
    ...
    relay.precision_m = maxf(
        sanitize_precision(requested_precision_m),
        sanitize_precision(parent->precision_m));
...
float nf_report_weight(const NfReport *report, uint64_t now_tick) {
    ...
    const uint64_t lifetime = report->expiry_tick > report->origin_tick
        ...
        : 1u;
    const uint64_t age = now_tick > report->origin_tick
        ? now_tick - report->origin_tick
        : 0u;
    const float freshness = clamp01(1.0f - (float)age / (float)lifetime);
    const float hop_penalty = 1.0f / (1.0f + 0.18f * (float)report->hops);
    const float precision_penalty = 1.0f /
        (1.0f + 0.08f * maxf(0.0f, report->precision_m - NF_REPORT_MIN_PRECISION_M));

    return clamp01(report->confidence) *
        (0.35f + 0.65f * freshness) *
        hop_penalty *
        precision_penalty;
}
...
const char *nf_report_kind_name(NfReportKind kind) {
    switch (kind) {
        case NF_REPORT_ACTOR_POSITION: return "ACTOR_POSITION";
        case NF_REPORT_ROUTE_STATE: return "ROUTE_STATE";
        case NF_REPORT_RESOURCE_STATE: return "RESOURCE_STATE";
        case NF_REPORT_STASIS_STATE: return "STASIS_STATE";
        default: return "NONE";
    }
}
...
const char *nf_information_channel_name(NfInformationChannel channel) {
    switch (channel) {
        ...
        case NF_INFO_CHANNEL_AUDIBLE: return "AUDIBLE";
