...
static bool ray_aabb(NfVec3 origin,NfVec3 direction,NfVec3 minimum,NfVec3 maximum,float *distance){
    ...
    const float ov[3]={origin.x,origin.y,origin.z},dv[3]={direction.x,direction.y,direction.z};
    const float mn[3]={minimum.x,minimum.y,minimum.z},mx[3]={maximum.x,maximum.y,maximum.z};
    for(int i=0;i<3;++i){
    ...
    if(tmax<0.0f)return false;if(distance!=NULL)*distance=tmin>=0.0f?tmin:tmax;return true;
}

static bool ray_sphere(NfVec3 origin,NfVec3 direction,NfVec3 center,float radius,float *distance){
    NfVec3 oc=vsub(origin,center);float b=vdot(oc,direction),c=vdot(oc,oc)-radius*radius,disc=b*b-c;
    if(disc<0.0f)return false;float root=sqrtf(disc),t=-b-root;if(t<0.0f)t=-b+root;if(t<0.0f)return false;
    if(distance!=NULL)*distance=t;return true;
}

static void consider(bool hit,float distance,NfHitZone zone,float *nearest,NfHitZone *best){
    if(hit&&distance>=0.0f&&distance<*nearest){*nearest=distance;*best=zone;}
}

NfHitZone nf_cattler_hit_zone_for_ray(NfVec3 origin,NfVec3 direction,NfVec3 actor_base,float max_distance,float *distance_out){
    float nearest=max_distance,t=FLT_MAX;NfHitZone best=NF_HIT_NONE;bool hit=false;
    ...
    hit=ray_aabb(origin,direction,foot_min,foot_max,&t);consider(hit,t,NF_HIT_FOOT,&nearest,&best);
    ...
    t=FLT_MAX;hit=ray_aabb(origin,direction,knee_min,knee_max,&t);consider(hit,t,NF_HIT_KNEE,&nearest,&best);
    ...
    t=FLT_MAX;hit=ray_aabb(origin,direction,body_min,body_max,&t);consider(hit,t,NF_HIT_BODY,&nearest,&best);
    NfVec3 head={actor_base.x,actor_base.y+NF_CATTLER_HEAD_CENTER_Y,actor_base.z};
    t=FLT_MAX;hit=ray_sphere(origin,direction,head,NF_CATTLER_HEAD_RADIUS,&t);consider(hit,t,NF_HIT_HEAD,&nearest,&best);
    if(best!=NF_HIT_NONE&&distance_out!=NULL)*distance_out=nearest;return best;
}
