...
static void prepare_cattler_evidence_proof(
    NfCattlerSystem *cattlers,NfWorld *world,const NfActor *player,NfSemanticBus *semantics) {
    static const NfVec3 positions[2]={
        {24.0f,0.05f,8.0f},
        {-24.0f,0.05f,14.0f}
    };
    for(size_t i=0u;i<cattlers->count&&i<2u;++i) {
        ...
        /* Face away so this proof is explicitly audible evidence, not accidental
           line-of-sight knowledge. Both predatory STALK and rare non-predatory
           AVOID must still become embodied responses to legitimate evidence. */
        agent->yaw=atan2f(
            positions[i].x-player->transform.position.x,
            positions[i].z-player->transform.position.z);
...
static void embodied_population_contract(void) {
    ...
    prepare_cattler_evidence_proof(&cattlers,&world,player,&semantics);
    ...
    float cattler_control_peak=0.0f;
    for(unsigned tick=0u;tick<NF_TICK_RATE*5u;++tick) {
        ...
        for(size_t i=0u;i<cattler_count;++i) {
            const float magnitude=fabsf(cattler_controls[i].move.forward)+fabsf(cattler_controls[i].move.strafe);
            if(magnitude>cattler_control_peak)cattler_control_peak=magnitude;
    ...
    assert(cattler_control_peak>0.25f);
    assert(cattler_distance>0.50f);

    printf("[v1.2c] POPULATION PLAYER=1 TEAMMATE=1 RIVAL=%zu CATTLER=%zu\n",rivals.count,cattlers.count);
    printf("[v1.2c] TEAMMATE CONTROL=%.2f DISPLACEMENT=%.2fm | RIVAL=%.2fm | CATTLER CONTROL=%.2f DISPLACEMENT=%.2fm\n",
        teammate_control_peak,teammate_distance,rival_distance,cattler_control_peak,cattler_distance);
}
