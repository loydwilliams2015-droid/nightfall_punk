...
static void place_cattlers_in_slice(NfCattlerSystem *cattlers,NfWorld *world) {
    static const NfVec3 positions[NF_V14_CATTLERS]={
        {24.0f,0.05f,8.0f},
        {-24.0f,0.05f,14.0f}
    };
...
static void reset(
    ...
    NfClaimTable *interaction_claims,NfEntityId *player,NfVec3 *teammate_origin) {
    ...
    nf_claim_table_init(interaction_claims);
    *player=nf_world_spawn_actor(world,NF_FACTION_PLAYER,(NfVec3){0,.05f,-18});
    nf_teammate_init(teammate,world,true,world->seed^0x14u);
    nf_ai_init(rivals,world,NF_V14_RIVALS,world->seed^0xA114u);
    nf_ai_set_rival_relationship(rivals,NF_RELATION_HOSTILE);
    nf_cattler_init(cattlers,world,NF_V14_CATTLERS,NF_CATTLER_PROFILE_NORMAL,world->seed^0xCA771Eu);
...
static void proof_setup(
    NfWorld *world,NfRouteSystem *route,NfTeammateSystem *teammate,
    NfEntityId player,NfVec3 *teammate_origin,NfCameraState *camera,
    NfClaimTable *interaction_claims) {
    if(world==NULL||route==NULL||teammate==NULL||camera==NULL||route->open)return;
    NfActor *player_body=nf_world_find_actor(world,player);
    NfActor *teammate_body=nf_world_find_actor(world,teammate->actor_id);
    if(player_body==NULL||teammate_body==NULL)return;

    player_body->transform.position=(NfVec3){0.0f,0.05f,1.4f};
    player_body->transform.velocity=(NfVec3){0};
    teammate_body->transform.position=(NfVec3){-36.0f,0.05f,-30.0f};
    teammate_body->transform.velocity=(NfVec3){0};
    *teammate_origin=teammate_body->transform.position;

    teammate->memory=(NfTeammateRouteMemory){0};
    teammate->trace=(NfTeammateDecisionTrace){0};
    ...
    teammate->negative_control_logged=false;
    teammate->evidence_logged=false;
    teammate->last_route_report_id=0u;
...
static const char *route_report_status(const NfTeammateSystem *teammate,uint64_t tick,float *weight) {
    if(weight!=NULL)*weight=0.0f;
    if(teammate==NULL||teammate->last_route_report_id==0u)return "NONE";
    for(size_t i=0u;i<teammate->reports.count;++i) {
        const NfReport *report=&teammate->reports.reports[i];
        if(report->id!=teammate->last_route_report_id)continue;
        if(tick<report->deliver_tick)return "PENDING";
        if(tick>report->expiry_tick)return "EXPIRED";
        if(weight!=NULL)*weight=nf_report_weight(report,tick);
        return "LIVE";
    }
    return "NONE";
}
...
int main(void) {
    ...
    InitWindow(width,height,"nightfall!punk v1.4 relational combat support / bounded communications");
    SetTargetFPS(120);
    DisableCursor();
    ...
    NfClaimResult last_interaction_claim=NF_CLAIM_RESULT_INVALID;
    reset(&world,&route,&teammate,&rivals,&cattlers,&semantics,&interaction_claims,&player,&teammate_origin);
    ...
    while(!WindowShouldClose()) {
        ...
        if(IsKeyPressed(KEY_F4)) {
            proof_setup(&world,&route,&teammate,player,&teammate_origin,&camera,&interaction_claims);
            last_teammate_control=(NfControlFrame){0};
            last_interaction_claim=NF_CLAIM_RESULT_INVALID;
        }
        ...
        acc+=frame;
        while(acc>=dt) {
            nf_claim_expire(&interaction_claims,world.tick);
            nf_world_set_input(&world,player,player_input);

            const NfActor *player_tick=nf_world_find_actor_const(&world,player);
            const bool interact_ready=player_tick!=NULL&&!route.open&&player_input.interact_held&&
                nf_route_distance_to_gate(&route,player_tick->transform.position)<=NF_V12_ROUTE_INTERACT_RADIUS&&
                nf_contamination_manipulator_scale(player_tick)>=0.35f;
            if(interact_ready) {
                last_interaction_claim=nf_claim_try_acquire(
                    &interaction_claims,NF_CLAIM_INTERACTION,NF_CLAIM_HARD,
                    player,route.gate_key,world.tick,NF_TICK_RATE/3u,NULL);
            }

            const bool changed=nf_route_try_alter(&route,&world,player);
            if(changed) {
                (void)nf_claim_release(&interaction_claims,NF_CLAIM_INTERACTION,player,route.gate_key);
                ...
                (void)nf_teammate_publish_route_report(&teammate,&world,&route,player);
            }
        ...
        float crew_report_weight=0.0f;
        const char *crew_report_state=route_report_status(&teammate,world.tick,&crew_report_weight);
        const NfAiAgent *rival_a=rivals.count>0u?&rivals.agents[0]:NULL;
        const NfAiAgent *rival_b=rivals.count>1u?&rivals.agents[1]:NULL;

        DrawText("nightfall!punk v1.4 — RELATIONAL COMBAT SUPPORT / BOUNDED COMMUNICATIONS",20,14,20,RAYWHITE);
        DrawText("F1 PLAYER | F2 OBSERVER | F3 TOP | F4 PROOF SETUP | F5 RESET | WASD Shift Ctrl Space | hold E at purple gate",20,38,14,GRAY);
        DrawText(TextFormat("POP P %zu | TEAM %zu | RIVAL %zu (%zu moving) | CATTLER %zu (%zu moving)",
            ...
            faction_count(&world,NF_FACTION_RANCHER),moving_faction_count(&world,NF_FACTION_RANCHER)),20,60,15,RAYWHITE);
        DrawText(TextFormat("CAMERA %s | %s | anchor %.3fm | FOV %.1fV / %.1fH",
            ...
            camera.current_vertical_fov_deg,hfov),20,82,15,camera.mode==NF_CAMERA_FIRST_PERSON?SKYBLUE:GOLD);
        DrawText(TextFormat("OBJECT GATE key=%u rev=%u | %s | distance %.1fm | E %s | AFFORDANCE %s",
            route.gate_key,route.revision,nf_route_state_name(&route),gate_distance,e_held?"HELD":"UP",
            route.open?"CONSUMED":(affordance_ready?"READY":"NOT_READY")),20,104,15,
            affordance_ready?GREEN:(route.open?SKYBLUE:(Color){200,170,210,255}));
        DrawText(TextFormat("INTERACTION CLAIM %s | live=%zu | CREW REPORT #%u %s weight=%.2f",
            nf_claim_result_name(last_interaction_claim),nf_claim_live_count(&interaction_claims,world.tick),
            teammate.last_route_report_id,crew_report_state,crew_report_weight),20,126,15,GOLD);
        DrawText(TextFormat("TEAM CONTROL %.2f | SPEED %.2fm/s | DISPLACEMENT %.2fm | EVIDENCE %s | BELIEF %s | COMMIT %s",
            teammate_control,teammate_speed,teammate_displacement,nf_teammate_evidence_name(teammate.trace.evidence),
            teammate.memory.known_open?"OPEN":"UNCHANGED",nf_teammate_commitment_name(teammate.commitment)),20,148,15,
            teammate_displacement>1.0f?GREEN:YELLOW);
        DrawText(TextFormat("TEAM GRADIENT %.2f | ENERGY A %.1f B %.1f | UTILITY A %.2f B %s",
            teammate.trace.hotspot_gradient,teammate.trace.route_a_energy,teammate.trace.route_b_energy,
            teammate.trace.route_a_utility,route_b_utility),20,170,15,RAYWHITE);
        DrawText(TextFormat("RIVAL REPORTS live=%zu | COVER CLAIMS live=%zu | R1 %s %.2f | R2 %s %.2f",
            nf_ai_live_report_count(&rivals,world.tick),nf_ai_live_claim_count(&rivals,world.tick),
            rival_a?nf_ai_evidence_name(rival_a->knowledge.source):"NONE",rival_a?rival_a->knowledge.confidence:0.0f,
            rival_b?nf_ai_evidence_name(rival_b->knowledge.source):"NONE",rival_b?rival_b->knowledge.confidence:0.0f),20,192,15,(Color){255,130,130,255});
        DrawText(TextFormat("R1 report=%u ancestry=%u hops=%u exposure=%.2f | R2 report=%u ancestry=%u hops=%u exposure=%.2f",
            rival_a?rival_a->knowledge.report_id:0u,rival_a?rival_a->knowledge.report_ancestry_id:0u,rival_a?(unsigned)rival_a->knowledge.report_hops:0u,rival_a?rival_a->selected_cover_exposure:1.0f,
            rival_b?rival_b->knowledge.report_id:0u,rival_b?rival_b->knowledge.report_ancestry_id:0u,rival_b?(unsigned)rival_b->knowledge.report_hops:0u,rival_b?rival_b->selected_cover_exposure:1.0f),20,214,14,(Color){255,170,170,255});
        DrawText("CAUSAL CHAIN: WORLD REVISION -> EVIDENCE/REPORT -> BELIEF -> UTILITY -> CLAIM -> CONTROL -> BODY -> NEW WORLD RELATION",20,238,14,GOLD);
        DrawText("white Player | chrome-blue teammate | red Human Rivals | orange/yellow Dream Cattlers",20,260,14,GRAY);
        DrawText("v1.4 proof: bounded information + temporal claims around proven combat/movement; player weapons/viewmodel remain v1.5",20,688,14,GRAY);
        EndDrawing();
    }
